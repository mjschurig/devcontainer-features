...
install_debian_dependencies() {
    ...
    # Check if we need to install a newer GCC for Trilinos/Kokkos compatibility
    # Trilinos/Kokkos requires GCC 8.2.0 or higher
    check_and_install_newer_gcc() {
        local current_gcc_version=""
        local needs_newer_gcc=false

        # Get current GCC version if available
        if command -v gcc >/dev/null 2>&1; then
            current_gcc_version=$(gcc -dumpversion | cut -d. -f1)
            if [ "$current_gcc_version" -lt 8 ]; then
                needs_newer_gcc=true
            fi
        else
            # No GCC installed, we'll need a newer one
            needs_newer_gcc=true
        fi

        # Check for specific Ubuntu versions known to have older GCC
        if [ "$OS" = "ubuntu" ] && [ "$OS_VERSION" = "18.04" ]; then
            needs_newer_gcc=true
        elif [ "$OS" = "ubuntu" ] && [ "$OS_VERSION" = "16.04" ]; then
            needs_newer_gcc=true
        elif [ "$OS" = "debian" ] && [ "$OS_VERSION" = "9" ]; then
            needs_newer_gcc=true
        fi

        if [ "$needs_newer_gcc" = "true" ]; then
            log_info "Detected older GCC version ($current_gcc_version) - installing newer GCC for Trilinos/Kokkos compatibility..."
            ...
            # Install GCC 9 (which meets the >= 8.2.0 requirement)
            DEBIAN_FRONTEND=noninteractive apt-get install -y \
                gcc-9 \
                g++-9 \
                gfortran-9

            # Set GCC 9 as the default with high priority
            update-alternatives --install /usr/bin/gcc gcc /usr/bin/gcc-9 90 \
                               --slave /usr/bin/g++ g++ /usr/bin/g++-9 \
                               --slave /usr/bin/gfortran gfortran /usr/bin/gfortran-9

            # Also install the default versions as fallback if they exist
            if [ -n "$current_gcc_version" ] && [ "$current_gcc_version" -ge 4 ]; then
                DEBIAN_FRONTEND=noninteractive apt-get install -y \
                    gcc-${current_gcc_version} \
                    g++-${current_gcc_version} \
                    gfortran-${current_gcc_version} || true

                update-alternatives --install /usr/bin/gcc gcc /usr/bin/gcc-${current_gcc_version} 70 \
                                   --slave /usr/bin/g++ g++ /usr/bin/g++-${current_gcc_version} \
                                   --slave /usr/bin/gfortran gfortran /usr/bin/gfortran-${current_gcc_version} || true
            fi

            log_info "GCC 9 installed and set as default for Trilinos/Kokkos compatibility"

            # Verify the installation
            if command -v gcc >/dev/null 2>&1; then
                local new_version=$(gcc --version | head -n1)
                log_info "Active GCC version: $new_version"
            fi
        else
            log_info "GCC version $current_gcc_version is sufficient for Trilinos/Kokkos"
        fi
    }

    # Only check for newer GCC if Trilinos is enabled
...
EOF
    ...
    if [ "$ENABLE_TRILINOS" = "true" ]; then
        log_warn "Trilinos requires GCC 8.2+ (automatically handled) - may still have build issues on some systems"
